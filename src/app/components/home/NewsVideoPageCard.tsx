...
export function VideoPageCard({ item }: { item: NewsVideoPageItem }) {
  ...
  const thumbSrc = hasImage ? item.image : (getYouTubeThumbnail(item.url) || "/images/news/news-default-banner.png");
  ...
  return (
    <div className="nv-page-card">
      ...
      <div className="nv-page-card-body">
        ...
        <p ref={summaryRef} className={`nv-card-summary${expanded ? " nv-card-summary-expanded" : ""}`} style={{ color: "var(--text-secondary)", lineHeight: 1.6, marginBottom: "1rem" }}>
          {text}
        </p>
